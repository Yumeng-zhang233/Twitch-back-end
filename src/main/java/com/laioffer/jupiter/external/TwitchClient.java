...
public class TwitchClient {
    private static final String TOKEN = "Bearer n5ccuxj2gxlyf3n5lvzqry39cbhaii";
    private static final String CLIENT_ID = "4ruu6rp6ibtzcqax0itptwxjfqs3yp";
    ...
    //Twitch client 功能
    //Implement buildGameURL function. It will help generate the correct URL
    //when you call Twitch Game API.
    //Build request URL
    private String buildGameURL(String url, String gameName, int limit){
        // Build the request URL which will be used
        // when calling Twitch APIs, e.g. https://api.twitch.tv/helix/games/top
        // when trying to get top games.

        if(gameName.isEmpty()){
            return String.format(url,limit);//return 第二个 above url
        }else{
            try{
    ...
    // Send HTTP request to Twitch Backend based on the given URL, and
    // returns the body of the HTTP response returned from Twitch backend.

    private String searchTwitch(String url) throws TwitchException {
        CloseableHttpClient httpclient = HttpClients.createDefault();

        // Define the response handler to parse and return HTTP response body
    ...
    /*
    Finally, add the getGameList method to convert Twitch returned data
    to a list of Game objects. Then, provide two public methods topGames
    and searchGame to return the top game list, or a dedicated game.
     */
        private List<Game> getGameList(String data) throws TwitchException{
            // Convert JSON format data returned from Twitch to an Arraylist of
            // Game objects
    ...
    // Integrate search() and getGameList() together, returns the top x popular
    // games from Twitch.
    ...
    // Similar to buildGameURL, build Search URL that will be used
    // when calling Twitch API. e.g. https://api.twitch.tv/helix/clips?game_id=12924.
    private String buildSearchURL(String url, String gameId, int limit) throws TwitchException{
            try{
                gameId = URLEncoder.encode(gameId,"UTF-8");//处理特殊符号
            } catch (UnsupportedEncodingException e) {
                e.printStackTrace();
                throw new TwitchException("Can not encode gameId:" + gameId);
            }
    ...
    //通过build好的url 像twitch去发送请求
    // Returns the top x streams based on game ID.
    private List<Item> searchStreams(String gameId, int limit) throws TwitchException {
        List<Item> streams =
                getItemList(//3.转成item的list
                        searchTwitch(//2.给他一个URLreturn一个data形式的serch string
                                buildSearchURL(STREAM_SEARCH_URL_TEMPLATE, gameId, limit)));//1.build一个url
    ...
    public List<Item> searchByType(String gameId, ItemType type, int limit) throws TwitchException{
            ...
            switch(type){
                case STREAM:
                    ...
                    break;//otherwise fall through(继续执行下一行)
                case VIDEO:
