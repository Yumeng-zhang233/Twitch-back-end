...
public class ItemRecommender {
    ...
    private static final int DEFAULT_PER_GAME_RECOMMENDATION_LIMIT = 10;//推荐的相关游戏10个
    ...
    private List<Item> recommendByTopGames(ItemType type, List<Game> topGames) throws RecommendationException {
        ...
        for(Game game : topGames) {
            ...
            try {
                ...
                e.printStackTrace();
                throw new RecommendationException("Failed to get recommendation results");
            }
            for (Item item : items) {
    ...
    // [Stream, Video, Clip]. All items are related to the items previously
    ...
    private List<Item> recommendByFavoriteHistory(
        ...
        sortedFavoriteGameIdListByCount.sort((Map.Entry<String, Long> e1, Map.Entry<String, Long> e2) -> Long
                .compare(e2.getValue(), e1.getValue()));//descending, e1.getValue(), e2.getValue() ascending
        ...
        for (Map.Entry<String, Long> favoriteGame : sortedFavoriteGameIdListByCount) {
            ...
            for (Item item : items) {
                ...
                if (!favoritedItemIds.contains(item.getId())) {
                    recommendedItems.add(item);//如果该物品已经在favorite history，不需要再添加一次
                }
            }
        }
        return recommendedItems;
    ...
    // Keys of the may are [Stream, Video, Clip]. Each key is corresponding
    // to a list of Items objects, each item object is a recommended item based on
    ...
    public Map<String, List<Item>> recommendItemsByUser(String userId) throws RecommendationException {
      ...
      try(MySQLConnection conn = new MySQLConnection()){
          ...
          e.printStackTrace();
          throw new RecommendationException("Faild to get user favorite history for recommendation");
        }
        for (Map.Entry<String, List<String>> entry : favoriteGameIds.entrySet()) {
            if (entry.getValue().size() == 0) {//如果用户的favorite list是空的，从topgame拿资源
                TwitchClient client = new TwitchClient();
                List<Game> topGames;
                try {
    ...
    public Map<String, List<Item>> recommendItemsByDefault() throws RecommendationException {
        ...
        try{
            ...
            e.printStackTrace();
            throw new RecommendationException("Failed to get top game data for recommendations");
        }
